...
async def got_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("🔐 Send your *2FA password*:", parse_mode="Markdown")
        return WAIT_2FA
    except Exception as e:
...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dl_progress = ProgressMessage(query.message, "⬇️ Downloading")
    await dl_progress.start()

    file = None
    try:
        # Thread-safe progress bridge (yt-dlp runs in executor thread)
        loop = asyncio.get_event_loop()

        def sync_progress(percent, speed, downloaded, total):
            asyncio.run_coroutine_threadsafe(
                dl_progress.update(percent, speed, downloaded, total),
                loop
            )

        file = await download_video(link, client, progress_cb=sync_progress)
        await dl_progress.done("Download complete!")

    except Exception as e:
        await dl_progress.error(str(e))
        user_links.pop(user_id, None)
        return

    # ── Step 2: Watermark (if chosen) ─────────────────────────────────────
    if query.data == "wm_on":
        wm_progress = ProgressMessage(query.message, "🖊 Adding Watermark")
        await wm_progress.start()
        try:
            file = await loop.run_in_executor(None, add_watermark, file)
            await wm_progress.done("Watermark added!")
        except Exception as e:
            await wm_progress.error(f"Watermark failed: {e}")
            user_links.pop(user_id, None)
            return

    # ── Step 3: Upload with live progress ─────────────────────────────────
    up_progress = ProgressMessage(query.message, "📤 Uploading to Channel")
    await up_progress.start()

    try:
        async def upload_progress_cb(sent, total):
            await up_progress.update(sent / total * 100, 0, sent, total)
        ...
        await up_progress.done(f"[View post]({post_link})")

    except Exception as e:
        await up_progress.error(f"Upload failed: {e}")
